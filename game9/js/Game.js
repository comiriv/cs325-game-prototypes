...
GameStates.makeGame = function( game, shared ) {
    ...
    function onDown(cardeck){
        ...
        if(p1_first){
            if(turn==1){
                ...
                player2.turn_time = MAX_TIME;
                timer.add(1000, updateTime, this);
    ...
    function duel(){
        ...
        if(player1.stun){
            player1.stun = false;
            player2.attack = 0;
            maintext.text += "PLAYER 2's ATTACK WAS STUNNED BY lightning\n";
        }
        if(player2.stun){
            player2.stun = false;
            player1.attack = 0;
            maintext.text += "PLAYER 1's ATTACK WAS STUNNED BY lightning\n";
        }
        if(player1.freeze){
            player1.freeze = false;
            player2.attack = 0;
            maintext.text += "PLAYER 2's DEFENSE WAS FROZEN by ice\n";
        }
        if(player2.freeze){
            player2.freeze = false;
            player1.attack = 0;
            maintext.text += "PLAYER 1's DEFENSE WAS FROZEN by ice\n";
        }
        if(player1.project){
            player1.project = false;
            player2.attack = temp1;
            player2.defense = temp2
            player2.attack = player1.attack;
            player2.defense = player1.defense;
            player1.attack = temp1;
            player1.defense = temp2;
            maintext.text += "PLAYER 1 and 2's ATTACK AND DEFENSE SWITCHED\n";
        }
        if(player2.project){
            player2.project = false;
            player1.attack = temp1;
            player1.defense = temp2
        ...
        timer.add(3000, resetPlayers, this);
    }
     ...
     function updateTime(){
        ...
        if(player1.turn_time > 0){
            ...
            if(player1.turn_time == 0){
                ...
                timer.add(1000, updateTime, this);
                maintext.text = player1.turn_time;
            }
        }
        else if(player2.turn_time > 0){
            player2.turn_time--;
            if(player2.turn_time == 0){
                onDown(fulldeck);
            }
            else{
                
                timer.add(1000, updateTime, this);
                maintext.text = player2.turn_time;
            }
        }
